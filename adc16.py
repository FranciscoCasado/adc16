...
class ADC16:  #katcp.RoachClient:
    ...
    def set_demux_fpga(self, fpga_demux):
        # Setting fpga demux rearranges the bits before they're output from the adc block
        # depending on the adc demux mode used.
        demux_shift = 24
        # state is assigned according to the adc16_controller memory map. (4+n) shifted by
        # the amount of bits that precede the WMM field. 4 always activates the W bit to
        # allow writing to the MM bits and n is determined by the adc demux mode used
        if fpga_demux == 1:
    ...
    def read_ram(self, device):
        ...
        # Part of the read request is the size parameter,1024, which specifies the amount
        # of bytes to read form the device
        """
    ...
    # The ADC16 controller word (the offset in write_int method) 2 and 3 are for delaying taps of A and B lanes,
    # respectively. Refer to the memory map word 2 and word 3 for clarification. The memory map was made for a
    # ROACH design so it has chips A-H.
    ...
    # Returns an array of error counts for a given tap(assume structure chan 1a, chan 1b, chan 2a, chan 2b etc..
    # until chan 4b. Taps argument can have a value of an int or a string. If it's a string then it will iterate
    # through all 32 taps. If it's an int it will only delay all channels by that particular tap value.
    ...
    def walk_taps(self):
        for chip, chip_num in self.chips.iteritems():
            # Set demux 4 on the FPGA side
            # (just rearranging outputs as opposed to dividing clock and assigning channels)
            ...
            for i in range(8):
                if not(error_counts_0[0][i]) or not(error_counts_31[0][i]):
                    ...
                    error_counts_31 = self.test_tap(chip_num, 31)
            """
            # Error_list is a list of 32 'rows'(corresponding to the 32 taps), each row containing 8 elements,
            # each element is the number of errors of that lane  when compared to the expected value.
            # read_ram method unpacks 1024 bytes. There are 8 lanes so each lane gets 1024/8=128 read outs from
            # a single call to read_ram method, like this, channel_1a etc. represent the errors in that channel
            ...
            min_tap = []
            max_tap = []
            # This loop goes through error_list, finds the elements with a value of 0 and appends them to the good tap
            # range list. It also picks out the elements corresponding to different channels and groups them together.
            # The error_list is a list where each 'row' is a different tap. I wanted to find the elements in each
            # channel that have zero errors, group the individual channels, and get the value of the tap in which
            # they're in - which is the index of the row
            for i in range(8):
